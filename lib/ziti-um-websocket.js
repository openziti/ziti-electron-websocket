...
const ZitiSocket = require('./ziti-um-socket').ZitiSocket;
const SessionCookies = require('./session-cookies').SessionCookies;

let ziti;
try {
  logger.info('about to load ziti-sdk-nodejs');
  ziti = require('ziti-sdk-nodejs');
  logger.info('ziti is: %o', ziti);
  require('assert').equal(ziti.ziti_hello(), 'ziti');
  logger.info('ziti.ziti_hello says: %s', ziti.ziti_hello());
} catch (e) {
  logger.error(e);
}
...
class ZitiWebSocket extends EventEmitter {
  ...
  constructor(address, protocols, options) {
    ...
    logger.debug('ZitiWebSocket ctor entered for url: %o', address);
  ...
  async ziti_enroll() {
    return new Promise((resolve, reject) => {
      logger.debug('ziti_enroll entered');
      ...
      logger.debug('Now calling ziti.ziti_enroll: ' + ziti.ziti_enroll);

      const erc = ziti.ziti_enroll(identityPath, (data) => {
        logger.debug('ziti.ziti_enroll callback entered, data is (%o)', data);
      ...
      logger.debug('ziti.ziti_enroll rc is (%o)', erc);
  ...
  async ziti_init() {
    ...
    return new Promise(async (resolve, reject) => {
      logger.debug('ziti_init entered');
      ...
      if (!fs.existsSync(this.getIdentityFilePath())) {
        await this.ziti_init().catch((err) => {
          reject(new Error('Ziti init failed rc [' + err + ']'));
        });
      }

      const rc = ziti.ziti_init(
        this.getIdentityFilePath(),

        (cbRC) => {
          logger.debug('ziti.ziti_init callback entered, cbRC is (%o)', cbRC);
      ...
      logger.debug('ziti.ziti_init rc is (%o)', rc);
  ...
  async doZitiInitialization() {
    ...
    try {
      if (!this._zitiInitialized) {
        await this.ziti_init().catch((e) => {
          logger.error('ziti_init exception: ' + e);
        });
   ...
   * @return  Promise
   */
  callNativeZitiServiceAvailable(service) {
    return new Promise((resolve) => {
    ...
    return new Promise((resolve, reject) => {
      memoryCache.get(service, async (err, cachedResult) => {
        ...
        if (cachedResult) {
          ...
          const newResult = await this.callNativeZitiServiceAvailable(
            service
          ).catch((e) =>
            console.log('callNativeZitiServiceAvailable Error: ', e.message)
          );
  ...
  setSocket(socket, head, maxPayload) {
    logger.debug('setSocket() entered, socket: %o', socket);
...
async function initAsClient(websocket, address, protocols, options) {
  ...
  logger.debug('serviceIsAvailable is: %o', serviceIsAvailable);
  ...
  if (serviceIsAvailable) {
    if (serviceIsAvailable.status === 0) {
      const socket = new ZitiSocket(ziti, opts);
      if (await socket.connect(address)) {
        websocket.setSocket(socket, [], opts.maxPayload);
      } else {
        websocket.readyState = ZitiWebSocket.CLOSING;
        websocket.emit('error', new Error('cannot get connection'));
        websocket.emitClose();  
      }
    }
  } else {
    ...
    logger.info('WebSocket handshake request has been sent: %o', websocket._req);
    ...
    req.on('error', (err) => {
      logger.error('req.on.error %o', err);
    ...
    req.on('upgrade', (res, socket, head) => {
      logger.info('WebSocket handshake on.upgrade \nsocket=[%o] \nHTTP Response=[%o]', socket, res);
      ...
      if (protError) {
        logger.error('protError=%o', protError);
...
function zitiConnect(options) {
  logger.debug('zitiConnect() entered: %o', options);
  ...
  const tlsSocket = tls.connect(options, () => {
    logger.info('tls.connect', tlsSocket.authorized ? 'authorized' : 'unauthorized');  
    logger.info('Cipher: ' + JSON.stringify(tlsSocket.getCipher()));
    logger.info('Protocol: ' + JSON.stringify(tlsSocket.getProtocol()));
  });

  logger.info('zitiConnect(): TLSSocket is now wrapped around ZitiSocket: \n%o', tlsSocket);
...
function receiverOnMessage(data) {
  logger.info('receiverOnMessage entered, data is: %o', data);
...
function receiverOnPing(data) {
  logger.info('receiverOnPing entered, data is: %o', data);
...
function receiverOnPong(data) {
  logger.info('receiverOnPong entered, data is: %o', data);
